...
def rucksack_sort2() -> list:
	"""
	Find the character that appear in all three compartments of each rucksack and appends them to a list
	"""
	apu = read_input()
	rucksack_list = []
	for i in range(0, len(apu), 3):
		first_compartment = apu[i]
		second_compartment = apu[i+1]
		third_compartment = apu[i+2]
		for j in first_compartment:
			if j in second_compartment and j in third_compartment:
				rucksack_list.append(j)
				break
	return rucksack_list
if __name__ == '__main__':
	print(f"Part 1 Priority sum: {sum_priorities(rucksack_sort())}") # Part 1
	print(f"Part 2 Priority sum: {sum_priorities(rucksack_sort2())}") # Part 2